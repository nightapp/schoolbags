...
$sql="SELECT * FROM work_point_topics,users,classlist,classlistofusers WHERE classlist.classID=classlistofusers.classID AND classlist.classID=work_point_topics.class_id AND classlistofusers.studentID=".$_SESSION["userID"]." AND (users.schoolID='".$_SESSION["schoolID"]."' || users.schoolID=0) ".$add."AND users.userID=work_point_topics.topicOwner ORDER BY date DESC LIMIT ".$from.",11";
