...
<div class="subHeading" style="width:100%">View Copy</div>
<?php
if($_POST["filePath"]){
...
//	echo $sql;
	...
	echo '<a href="'.$userTypePage[$_SESSION["userType"]].'?subPage=journal&iframePage=doHomework&date='.$fileDate.'&class='.$class.'&subject='.$row["subject"].'&teacher='.$row["FirstName"]." ".$row["LastName"].'&extraRef='.$row["extraRef"].'&folder='.$topic.'">Edit this copy</a>';
	$file="../../../".$_SESSION["schoolPath"].$type."/".$studentID."/homework/".$_POST["filePath"];
	$file=str_replace("//","/",$file);
//	echo $file;
	$fp=fopen($file,"r");
	$contents=html_entity_decode(fread($fp,filesize($file)));
	echo "<div style='text-align:center;width:100%'>".$contents."</div>";
