...
/*		$('textarea.tinymce').tinymce({
		...
		$("#addPost").submit(function(){
			if($("#elm1").val()=="Type post text") return false;
...
//			alert("K");
			$("#elm1").val("Type post text");
		}
		
				
		$("#elm1").focus(function(){
			if($(this).val()=="Type post text"){
				$(this).val("");
			}
		}).blur(function(){
			if($(this).val()==""){
				$(this).val("Type post text");
			}
		});
});
</script>
...
if(isset($_POST["text"])){
	...
	// echo $_POST["text"];
		$created="Updated";
		$sql="UPDATE posts SET text='".addslashes(urldecode(stripslashes($_POST["text"])))."'".$changeFile." WHERE topicID=".$_POST["topic"]." AND date='".$_POST["date"]."' AND postOwner=".$_SESSION["userID"];
	} else {
		$sql="INSERT INTO posts SET text='".addslashes(urldecode(stripslashes($_POST["text"])))."', topicID=".$_POST["topic"].", date='".$date."', postOwner=".$_SESSION["userID"].", schoolID=".$_SESSION["schoolID"].$changeFile;

	}
	$result=mysql_query($sql);
?>
<div id="noticeBoardHeader" class="subHeading">Post <?php echo $created;?></div>
<?php
} else {
?>
<div id="noticeBoardHeader" class="subHeading"><?php echo (!isset($_GET["date"]))?"Add":"Edit";?> Post</div>
<form id="addPost" method="post" action="teacherPage.php?subPage=addPost&noRedirect=true" enctype="multipart/form-data">
<?php
$text="";
$row=array();
...
$addreplace="Attach";
if(isset($_GET["date"])){
	$sql="SELECT * from posts WHERE date='".urldecode($_GET["date"])."' AND topicID='".$_GET["topic"]."' AND postOwner=".$_SESSION["userID"];
	$result=mysql_query($sql);
